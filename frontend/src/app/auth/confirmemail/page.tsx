...
import '@/styles/spinner.css'; // Import file CSS cho spinner
...
const ConfirmEmailPage = () => {
  ...
  // Hàm xóa các ô nhập OTP
  const clearOTPInputs = () => {
    otpRefs.current.forEach(input => {
      if (input) input.value = '';
    });
    otpRefs.current[0]?.focus(); // Focus vào ô đầu tiên sau khi xóa
  };
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(errorData.message); // Hiển thị thông báo "OTP không hợp lệ"
        clearOTPInputs(); // Xóa các ô nhập OTP
        throw new Error(errorData.message || 'Xác thực OTP thất bại');
      }

      const result = await response.json();
      setSuccess(true);
      console.log('Verification successful, User ID:', result.userId);
      setTimeout(() => {
