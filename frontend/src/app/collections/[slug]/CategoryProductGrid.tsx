...
const CategoryProductGrid: React.FC<CategoryProductGridProps> = ({ categoryName, slug }) => {
  ...
  const { filters, updateMetadata, updateFilters } = useFilters();
  const [showWishlistNotification, setShowWishlistNotification] = useState(false);
  const pageSize = 25;

  // Set category filter khi component mount
  useEffect(() => {
    if (categoryName && !filters.category.includes(categoryName)) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        // Add category filter - ưu tiên categoryName từ props
    ...
    <>
      ...
      {!isLoading && !error && products.length === 0 && (
        <div className="no-products" style={{ padding: '60px 0', textAlign: 'center' }}>
          <p style={{ fontSize: '18px', color: '#999' }}>Không tìm thấy sản phẩm nào trong danh mục này.</p>
        </div>
      )}
      {!isLoading && !error && products.length > 0 && (
        <>
